...
*   **Condition:** Triggered if validation fails (and attempt count < 3) within the Wizard workflow (specifically if `validate_code` node is used). *Note: The provider system also has its own refinement loop (see Part 2).*
