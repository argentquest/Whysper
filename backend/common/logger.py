...
class ConsoleFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Build base message
        base_msg = f"{color}[{timestamp}] {record.levelname:8} " f"{record.name:15} | {record.getMessage()}{reset}"
...
class CodeChatLogger:
    ...
    def _setup_handlers(self):
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            structured_log_file, maxBytes=10 * 1024 * 1024, backupCount=10, encoding="utf-8"  # 10MB
        )
        ...
        error_handler = logging.handlers.RotatingFileHandler(
            error_log_file, maxBytes=5 * 1024 * 1024, backupCount=5, encoding="utf-8"  # 5MB
        )
        ...
        perf_handler = logging.handlers.RotatingFileHandler(
            perf_log_file, maxBytes=5 * 1024 * 1024, backupCount=3, encoding="utf-8"  # 5MB
        )
...
class LoggerContextManager:
    ...
    def __enter__(self):
        ...
        self.old_context = LogContext(
            ...
            additional_data=(
                self.logger.context.additional_data.copy() if self.logger.context.additional_data else None
            ),
        )
