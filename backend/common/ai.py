...
class AIProviderFactory:
    ...
    _STATIC_PROVIDERS = {
        k: v for k, v in {"openrouter": OpenRouterProvider, "custom": CustomProvider}.items() if v is not None
    }
        ...
        for provider_name in provider_names:
            if provider_name in cls._STATIC_PROVIDERS:
                ...
                try:
                    ...
                    if hasattr(provider_class, "__bases__") and any(
                        "BaseAIProvider" in str(base) for base in provider_class.__bases__
                    ):
