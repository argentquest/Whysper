...
class DocumentationService:
    ...
    def _initialize_ai_processor(self):
        ...
        try:
            ...
            if api_key:
                ...
                self.logger.info(
                    "API key not configured in environment or settings, documentation generation will be limited"
                )
                self.ai_processor = None
                ...
                for item in node.body:
                    if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        method = {
                            ...
                            "decorators": (
                                [ast.unparse(d) for d in item.decorator_list] if hasattr(ast, "unparse") else []
                            ),
                        }
                    ...
                    if isinstance(item, ast.Assign):
                        for target in item.targets:
                            if isinstance(target, ast.Name):
                                attributes.append({"name": target.id, "line_number": item.lineno})
    ...
    def _extract_python_comments(self, content: str) -> List[Dict[str, Any]]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if stripped.startswith("#"):
                comments.append({"content": stripped[1:].strip(), "line_number": i, "type": "comment"})
    ...
    def _analyze_python_relationships(self, tree: ast.AST) -> Dict[str, List[str]]:
        ...
        relationships = {"imports": [], "inheritance": [], "dependencies": []}
    ...
    def _calculate_python_complexity(self, tree: ast.AST) -> Dict[str, Any]:
        ...
        metrics = {"lines_of_code": len(tree.body), "functions": 0, "classes": 0, "complexity_score": 0}
    ...
    def _extract_js_functions(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(arrow_pattern, content):
            func_name = match.group(1)
            functions.append(
                {"name": func_name, "line_number": content[: match.start()].count("\n") + 1, "is_arrow": True}
            )
    ...
    def _extract_js_variables(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(var_pattern, content):
            ...
            if not var_name.isupper():  # Skip constants
                variables.append({"name": var_name, "line_number": content[: match.start()].count("\n") + 1})
    ...
    def _extract_js_constants(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(const_pattern, content):
            ...
            if const_name.isupper():  # Only include uppercase constants
                constants.append({"name": const_name, "line_number": content[: match.start()].count("\n") + 1})
    ...
    def _extract_js_docstrings(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(jsdoc_pattern, content, re.DOTALL):
            ...
            docstrings.append(
                {"content": cleaned_doc, "line_number": content[: match.start()].count("\n") + 1, "type": "jsdoc"}
            )
    ...
    def _extract_js_comments(self, content: str) -> List[Dict[str, Any]]:
        ...
        for i, line in enumerate(lines, 1):
            stripped = line.strip()
            if stripped.startswith("//"):
                comments.append({"content": stripped[2:].strip(), "line_number": i, "type": "line_comment"})
    ...
    def _analyze_js_relationships(self, content: str) -> Dict[str, List[str]]:
        ...
        relationships = {"imports": self._extract_js_imports(content), "inheritance": [], "dependencies": []}
    ...
    def _calculate_js_complexity(self, content: str) -> Dict[str, Any]:
        ...
        # Simple complexity calculation based on control structures
        control_patterns = [r"\bif\b", r"\bfor\b", r"\bwhile\b", r"\bswitch\b", r"\btry\b", r"\bcatch\b"]
    ...
    def _extract_java_methods(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(method_pattern, content):
            ...
            if method_name in ["class", "interface", "enum", "i", "for", "while", "switch", "try", "catch", "finally"]:
                continue

            methods.append({"name": method_name, "line_number": content[: match.start()].count("\n") + 1})
    ...
    def _extract_java_variables(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(var_pattern, content):
            ...
            if var_name in ["class", "interface", "enum", "i", "for", "while", "switch", "try", "catch", "finally"]:
                continue
    ...
    def _analyze_java_relationships(self, content: str) -> Dict[str, List[str]]:
        ...
        relationships = {"imports": self._extract_java_imports(content), "inheritance": [], "dependencies": []}
    ...
    def _extract_go_functions(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(func_pattern, content):
            func_name = match.group(1)
            functions.append({"name": func_name, "line_number": content[: match.start()].count("\n") + 1})
    ...
    def _extract_go_variables(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(short_var_pattern, content):
            var_name = match.group(1)
            variables.append(
                {"name": var_name, "line_number": content[: match.start()].count("\n") + 1, "type": "short_declaration"}
            )
    ...
    def _extract_go_docstrings(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(godoc_pattern, content, re.DOTALL):
            ...
            if doc_content:
                docstrings.append(
                    {"content": doc_content, "line_number": content[: match.start()].count("\n") + 1, "type": "godoc"}
                )
    ...
    def _calculate_go_complexity(self, content: str) -> Dict[str, Any]:
        ...
        # Simple complexity calculation based on control structures
        control_patterns = [r"\bif\b", r"\bfor\b", r"\bswitch\b", r"\bselect\b", r"\bgo\b", r"\bdefer\b"]
    ...
    def _extract_rust_structs(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(struct_pattern, content):
            ...
            structs.append({"name": struct_name, "line_number": content[: match.start()].count("\n") + 1})
    ...
    def _calculate_rust_complexity(self, content: str) -> Dict[str, Any]:
        ...
        # Simple complexity calculation based on control structures
        control_patterns = [r"\bif\b", r"\bmatch\b", r"\bloop\b", r"\bwhile\b", r"\bfor\b", r"\basync\b"]
    ...
    def _extract_cpp_classes(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(class_pattern, content):
            class_name = match.group(1)
            classes.append({"name": class_name, "line_number": content[: match.start()].count("\n") + 1})
    ...
    def _extract_cpp_functions(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(func_pattern, content):
            ...
            # Skip common keywords
            if func_name in ["class", "struct", "i", "for", "while", "switch", "try", "catch"]:
                continue
    ...
    def _extract_cpp_variables(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(var_pattern, content):
            ...
            # Skip common keywords
            if var_name in ["class", "struct", "i", "for", "while", "switch", "try", "catch"]:
                continue
    ...
    def _extract_cpp_constants(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(define_pattern, content):
            const_name = match.group(1)
            constants.append(
                {"name": const_name, "line_number": content[: match.start()].count("\n") + 1, "type": "macro"}
            )
    ...
    def _analyze_cpp_relationships(self, content: str) -> Dict[str, List[str]]:
        ...
        relationships = {"imports": self._extract_cpp_includes(content), "inheritance": [], "dependencies": []}
    ...
    def generate_documentation(self, request: DocumentationRequest) -> DocumentationResult:
        ...
        if self.ai_processor:
            documentation_content = self._generate_with_ai(codebase_content, request, template)
        else:
            # Fallback to template-based generation
            documentation_content = self._generate_with_template(structures, request, template)
    ...
    def _format_agent_prompt(
        self, agent_prompt: str, codebase_content: str, request: DocumentationRequest, template: str
    ) -> str:
    ...
    def _generate_with_template(
        self, structures: List[CodeStructure], request: DocumentationRequest, template: str
    ) -> str:
    ...
    def _load_templates(self) -> Dict[str, Dict[str, Any]]:
        ...
        template_dir = os.path.join(os.path.dirname(__file__), "../../templates/documentation")
    ...
    def create_documentation_zip(
        ...
        try:
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                ...
                readme_content = """# Documentation Package
