...
class D2RenderService:
    ...
    def _find_d2_executable(self) -> Optional[str]:
        ...
        if d2_path:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.warning(
                        f"D2 executable configured in environment at {d2_path} but not working, falling back to auto-detection"
                    )
            except (
                ...
                logger.warning(
                    f"D2 executable configured in environment at {d2_path} but failed: {e}, falling back to auto-detection"
                )
        ...
        possible_paths = [
            ...
            os.path.join(
                os.getcwd(), "D2", "d2-v0.7.1", "bin", "d2.exe"
            ),  # Windows project
    ...
    def validate_d2_code(self, d2_code: str) -> Tuple[bool, str]:
        ...
        if len(d2_code) > self.MAX_D2_CODE_LENGTH:
            return (
                False,
                f"D2 code too large ({len(d2_code)} bytes). Maximum allowed: {self.MAX_D2_CODE_LENGTH} bytes",
            )
        ...
        try:
            ...
            logger.debug(
                f"D2 validation successful - output size: {len(result.stdout)} bytes"
            )
