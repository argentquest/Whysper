...
class DiagramSession:
    ...
    def __init__(self, session_id: str):
        ...
        self.update_queue: asyncio.Queue = (
            asyncio.Queue()  # Real-time updates
        )
...
class DiagramFactoryService:
    ...
    def _load_keywords(self):
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.entity_words)} entity words, "
            ...
            self.entity_words = [
                ...
                "api", "frontend", "backend", "client", "admin", "customer"
            ]
            self.action_words = [
                "login", "register", "create", "update", "delete", "process",
                "send", "receive", "authenticate", "authorize", "validate", "flow"
            ]
            self.structure_words = [
                "architecture", "workflow", "sequence", "hierarchy", 
                "relationship", "contains", "connects", "depends", 
    ...
    async def start_generation(self, initial_prompt: str,
        ...
        try:
            ...
            logger.info(
                f"Started diagram generation for session "
    ...
    def _assess_information_completeness(
        ...
        has_minimum_info = (info_score >= 2 and word_count >= 15) or (
            info_score >= 3
        )
    ...
    async def render_diagram(self, diagram_code: Optional[str] = None):
        ...
        try:
            code_to_render = (
                diagram_code if diagram_code is not None 
                else self.session.diagram_code
            )
