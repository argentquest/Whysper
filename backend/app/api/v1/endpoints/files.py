...
def get_folder_file_counts(request: FolderFileCountRequest):
    ...
    folder_infos = [FolderInfo(path=item["path"], fileCount=item["fileCount"]) for item in counts]
...
def get_top_folders():
    ...
    try:
        ...
        if os.path.exists(code_path):
            folders = [d for d in os.listdir(code_path) if os.path.isdir(os.path.join(code_path, d))]
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail="Unable to read file: unsupported encoding or binary file")
        except PermissionError:
            raise HTTPException(status_code=403, detail="Permission denied: cannot read file")

        return FileReadResponse(success=True, data={"path": file_path, "content": content, "size": len(content)})
...
@router.post("/save", response_model=FileSaveResponse, summary="Save file", description="Save content to a file.")
@log_method_call
...
@router.get("/", summary="List files", description="Get files from a directory for frontend compatibility.")
@log_method_call
            ...
            try:
                for item in os.listdir(dir_path):
                    ...
                    if os.path.isfile(item_path):
                        ...
                        # Add directory entry
                        files.append(
                            {"path": relative_path, "name": item, "size": 0, "isSelected": False, "type": "directory"}
                        )
        ...
        if recursive:
            ...
            for item in os.listdir(directory):
                ...
                if os.path.isfile(item_path):
                    stat = os.stat(item_path)
                    files.append(
                        {"path": item, "name": item, "size": stat.st_size, "isSelected": False, "type": "file"}
                    )
                elif os.path.isdir(item_path):
                    files.append({"path": item, "name": item, "size": 0, "isSelected": False, "type": "directory"})
        ...
        for file_data in request.files:
            try:
                ...
                if not all(key in file_data for key in ["name", "content"]):
                    logger.info("Skipping invalid file data: missing required fields")
                    continue
                ...
                uploaded_file = UploadedFileItem(
                    path=file_path, name=file_name, size=file_size, content=file_content, type="file", is_uploaded=True
                )
