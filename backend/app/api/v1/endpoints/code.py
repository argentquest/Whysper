...
from app.utils.code_extraction import extract_code_blocks_from_content, find_message_content
...
def extract_code_blocks(request: CodeExtractionRequest):
    ...
    try:
        ...
        if not message_content:
            return {"success": True, "data": [], "message": "No content found for message"}
        ...
        raise HTTPException(status_code=500, detail=f"Failed to extract code blocks: {str(e)}")
