...
def validate_d2_with_cli(d2_code: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        result = subprocess.run(["d2", temp_file_name, "-"], capture_output=True, text=True, check=True, timeout=10)
...
def save_error_file(
    test_id: int, test_name: str, description: str, diagram_code: str, error_msg: str, output_dir: str
) -> str:
...
def main():
    ...
    history_dirs = ["backend/tests/Diagrams/history", "backend/backend/tests/Diagrams/history"]
    ...
    for history_file in history_files:
        try:
            ...
            if is_valid:
                ...
                print(
                    f"  [FAIL] D2 code is invalid: {error_msg[:100]}..."
                    if len(error_msg) > 100
                    else f"  [FAIL] D2 code is invalid: {error_msg}"
                )
