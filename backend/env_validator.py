...
class EnvValidator:
    ...
    def _setup_default_rules(self):
        ...
        # API Configuration
        self.add_rule("API_KEY", self._validate_api_key, "OpenRouter or other AI API key", "api", required=True)

        self.add_rule("OPENROUTER_API_KEY", self._validate_openrouter_key, "OpenRouter API key (sk-or-...)", "api")
        ...
        self.add_rule(
            "PROVIDERS", self._validate_provider_list, "Comma-separated list of available AI providers", "api"
        )
        ...
        self.add_rule(
            "MODELS", self._validate_model_list, "Comma-separated list of available models", "models", required=True
        )
        ...
        self.add_rule(
            "TOP_P", lambda v: self._validate_float_range(v, 0.0, 1.0), "Top-p nucleus sampling (0.0-1.0)", "parameters"
        )
        ...
        self.add_rule(
            "WINDOW_SIZE", self._validate_window_size, "Window size in format WIDTHxHEIGHT (e.g., 1200x800)", "ui"
        )
        ...
        self.add_rule(
            "CURRENT_SYSTEM_PROMPT", self._validate_system_prompt_file, "Current system message file path", "system"
        )
        ...
        self.add_rule(
            "LOG_DIR", self._validate_log_directory, "Directory for log files (defaults to 'logs')", "logging"
        )
    ...
    def validate(self, var_name: str, value: str) -> ValidationResult:
        ...
        try:
            ...
            result = ValidationResult(is_valid=is_valid, error_message=error_msg, category=rule.category)
            ...
            return ValidationResult(
                is_valid=False, error_message=f"Validation error for {var_name}: {str(e)}", category=rule.category
            )
    ...
    def get_validation_summary(self, results: Dict[str, ValidationResult]) -> Dict[str, Any]:
        ...
        for var_name, result in results.items():
            if not result.is_valid:
                ...
                if (
                    result.category in ["api", "models"]
                    or self.rules.get(var_name, ValidationRule(None, "", "")).required
                ):
    ...
    def _validate_provider_list(self, value: str) -> Tuple[bool, str]:
        ...
        if invalid_providers:
            return (
                False,
                f"Invalid providers: {', '.join(invalid_providers)}. Valid options: {', '.join(valid_providers)}",
            )
