...
class ToolCommandPatternMatcher:
    ...
    def analyze_question(self, question: str, tool_vars: Dict[str, str] = None) -> PatternMatch:
        ...
        return PatternMatch(pattern=context, confidence=confidence, context=context, match_type=match_type)
    ...
    def get_analysis_details(self, question: str, tool_vars: Dict[str, str] = None) -> Dict:
        ...
        return {
            ...
            "programming_keywords": len(
                set(re.findall(r"\b\w+\b", question.lower())).intersection(self._programming_keywords)
            ),
            "has_file_references": any(pattern.search(question) for pattern in self._file_reference_patterns),
            "analysis_patterns_matched": sum(
                1 for pattern in self._code_analysis_patterns if pattern.search(question.lower())
            ),
        }
