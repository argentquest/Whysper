...
# Add paths to find provider_test_helper
...
def load_sample_files():
    ...
    samples = {"d2": [], "mermaid": [], "c4": [], "plantuml": []}
    ...
    for file in samples_dir.glob("*.d2"):
        with open(file, "r") as f:
            samples["d2"].append({"name": file.stem, "code": f.read().strip()})

    for file in samples_dir.glob("*.mmd"):
        with open(file, "r") as f:
            samples["mermaid"].append({"name": file.stem, "code": f.read().strip()})

    for file in samples_dir.glob("*.puml"):
        with open(file, "r") as f:
            samples["c4"].append({"name": file.stem, "code": f.read().strip()})
...
def test_provider_with_samples(provider_id, diagram_type, samples):
    ...
    for sample in samples:
        ...
        success, content, metadata = helper.render_with_provider(
            code=sample["code"], diagram_type=diagram_type, provider_id=provider_id, output_format="svg"
        )

        if success:
            passed += 1
            print("[PASS]")
            results.append({"name": sample["name"], "success": True, "content_length": len(content)})
        else:
            failed += 1
            print("[FAIL]")
            results.append({"name": sample["name"], "success": False, "error": content})
...
def main():
    ...
    with open(summary_file, "w") as f:
        json.dump({"timestamp": __import__("datetime").datetime.now().isoformat(), "results": all_results}, f, indent=2)
