"""
Validate all 25 C4 tests using the Kroki C4 Provider Endpoint
"""
...
def generate_diagram_with_llm(
    prompt: str, test_id: int, diagram_type: str = "c4", provider: str = "krokic4"
) -> Tuple[bool, str, str]:
...
def process_test(test_case: Dict[str, Any], output_dir: str, script_dir: str, provider: str) -> Dict[str, Any]:
    ...
    success, svg_or_error, validation_error = generate_diagram_with_llm(prompt, test_case["id"], "c4", provider)
    ...
    result["validation_error"] = validation_error if validation_error else "C4 Syntax is Valid"
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg in ["25", "test25"]:
            ...
            print("Usage: python validate_all_25_krokic4.py [25|50]")
            return
    else:
        test_file = "test25.json"
        test_label = "25"
    ...
    print(f"Kroki C4 Provider Renderer - Processing test{test_label} tests")
    print("=" * 60)
    ...
    test_cases = test_data["c4_capability_tests"]
    ...
    for test_case in test_cases:
        ...
        result = process_test(test_case, output_dir, script_dir, "krokic4")
