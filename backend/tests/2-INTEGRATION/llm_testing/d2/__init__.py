...
def generate_d2_diagram(prompt):
    ...
    try:
        ...
        result = subprocess.run(["python", "d2_generator.py", prompt], capture_output=True, text=True, check=True)
...
def validate_d2_syntax(diagram_content):
    ...
    try:
        ...
        result = subprocess.run(["d2", "check"], input=diagram_content, capture_output=True, text=True, check=True)
