...
def validate_structurizr_diagram(prompt_text):
    ...
    payload = {"diagram_type": DIAGRAM_TYPE, "diagram_source": sanitized_text}

    try:
        # Send POST request to Kroki API to validate and generate diagram
        response = requests.post(
            f"{KROKI_BASE_URL}/{DIAGRAM_TYPE}", json=payload, headers={"Content-Type": "application/json"}
        )
        ...
        if response.status_code == 200:
            ...
            return {"valid": True, "diagram": diagram_data, "message": "Structurizr diagram successfully generated"}
        else:
        ...
        return {"valid": False, "error": f"Request failed: {str(e)}", "exception": True}
