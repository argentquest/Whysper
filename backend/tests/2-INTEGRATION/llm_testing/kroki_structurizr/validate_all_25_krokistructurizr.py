...
def generate_diagram_with_llm(
    prompt: str, test_id: int, diagram_type: str = "structurizr", provider: str = "krokistructurizr"
) -> Tuple[bool, str, str]:
...
def process_test(test_case: Dict[str, Any], output_dir: str, script_dir: str, provider: str) -> Dict[str, Any]:
    ...
    success, svg_or_error, validation_error = generate_diagram_with_llm(
        prompt, test_case["id"], "structurizr", provider
    )
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg in ["25", "test25"]:
            ...
            print("Usage: python validate_all_25_krokistructurizr.py [25|50]")
            return
    else:
        test_file = "test25.json"
        test_label = "25"
    ...
    print("\n" + "=" * 60)
    print("STRUCTURIZR VALIDATION COMPLETE")
    print("=" * 60)
