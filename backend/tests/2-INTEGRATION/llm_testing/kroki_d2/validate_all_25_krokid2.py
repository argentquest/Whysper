"""
Validate all 25 D2 tests using the Kroki D2 Provider Endpoint
This script will:
...
def generate_diagram_with_llm(
    prompt: str, test_id: int, diagram_type: str = "d2", provider: str = "krokid2"
) -> Tuple[bool, str, str]:
    """
    ...
    Args:
        ...
        diagram_type (str): Type of diagram (d2, mermaid, etc.)
        provider (str): Provider name (krokid2, krokimermaid, etc.)
...
def process_test(test_case: Dict[str, Any], output_dir: str, script_dir: str, provider: str) -> Dict[str, Any]:
    ...
    success, svg_or_error, validation_error = generate_diagram_with_llm(prompt, test_case["id"], "d2", provider)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg in ["25", "test25"]:
            ...
            print("Usage: python validate_all_25_krokid2.py [25|50]")
            return
    else:
    ...
    print(f"Kroki D2 Provider Renderer - Processing test{test_label} tests")
    print("=" * 60)
    ...
    for test_case in test_cases:
        ...
        result = process_test(test_case, output_dir, script_dir, "krokid2")
