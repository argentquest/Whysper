...
class TestCreateConversation:
    ...
    async def test_create_conversation_with_title(self, sample_conversation_data, mock_conversation_service):
        ...
        result = await mock_conversation_service.create_conversation(user_id="user_123", title="Test Conversation")
    ...
    async def test_create_conversation_returns_id(self, mock_conversation_service):
        ...
        result = await mock_conversation_service.create_conversation(user_id="user_123", title="Test")
        assert "id" in result
        assert result["id"] == "conv_123"
    ...
    async def test_create_conversation_with_description(self, mock_conversation_service):
        ...
        result = await mock_conversation_service.create_conversation(
            user_id="user_123", title="Test", description="Test description"
        )
        assert result["id"] == "conv_123"
...
class TestGetConversationHistory:
    ...
    async def test_get_empty_history(self, mock_conversation_service):
        ...
        result = await mock_conversation_service.get_history(conversation_id="conv_123")
    ...
    async def test_get_history_with_pagination(self, mock_conversation_service):
        ...
        await mock_conversation_service.get_history(conversation_id="conv_123", limit=10, offset=0)
    ...
    async def test_get_history_ordering(self, mock_conversation_service):
        ...
        await mock_conversation_service.get_history(conversation_id="conv_123")
        mock_conversation_service.get_history.assert_called_once()
...
class TestSaveMessage:
    ...
    async def test_save_simple_message(self, mock_conversation_service, sample_message_data):
        ...
        result = await mock_conversation_service.save_message(
            conversation_id="conv_123", user_id="user_123", content="Hello, World!"
        )
    ...
    async def test_save_message_returns_message_id(self, mock_conversation_service):
        ...
        result = await mock_conversation_service.save_message(
            conversation_id="conv_123", user_id="user_123", content="Test message"
        )
        assert "id" in result
    ...
    async def test_save_message_with_metadata(self, mock_conversation_service):
        ...
        result = await mock_conversation_service.save_message(
            conversation_id="conv_123", user_id="user_123", content="Message with metadata", metadata={"key": "value"}
        )
        assert result["id"] == "msg_123"
    ...
    async def test_save_message_includes_timestamp(self, mock_conversation_service):
        ...
        mock_conversation_service.save_message.return_value = {"id": "msg_123", "created_at": datetime.now()}
        result = await mock_conversation_service.save_message(
            conversation_id="conv_123", user_id="user_123", content="Test"
        )
        assert "created_at" in result
    ...
    async def test_save_multiple_messages(self, mock_conversation_service):
        ...
        for i in range(5):
            await mock_conversation_service.save_message(
                conversation_id="conv_123", user_id="user_123", content=f"Message {i}"
            )
...
class TestDeleteConversation:
    ...
    async def test_delete_conversation(self, mock_conversation_service):
        """Delete conversation"""
        result = await mock_conversation_service.delete_conversation(conversation_id="conv_123")
    ...
    async def test_delete_nonexistent_conversation(self, mock_conversation_service):
        ...
        result = await mock_conversation_service.delete_conversation(conversation_id="nonexistent")
...
class TestSearchConversation:
    ...
    async def test_search_by_title(self, mock_conversation_service):
        ...
        await mock_conversation_service.get_conversations(user_id="user_123", search_query="Test")
    ...
    async def test_get_all_conversations(self, mock_conversation_service, conversation_list):
        ...
        result = await mock_conversation_service.get_conversations(user_id="user_123")
        assert len(result) >= 0
...
class TestConversationPermissions:
    ...
    async def test_owner_can_access(self, mock_conversation_service):
        ...
        await mock_conversation_service.get_conversation(conversation_id="conv_123", user_id="user_123")
    ...
    async def test_other_user_cannot_access(self, mock_conversation_service):
        ...
        result = await mock_conversation_service.get_conversation(conversation_id="conv_123", user_id="user_456")
...
class TestSessionState:
    ...
    async def test_concurrent_sessions(self, mock_conversation_service):
        ...
        conv1 = await mock_conversation_service.create_conversation(user_id="user_123", title="User 1 Conversation")
        conv2 = await mock_conversation_service.create_conversation(user_id="user_456", title="User 2 Conversation")
