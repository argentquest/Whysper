...
async def test_validate_mermaid_code():
    ...
    assert result.get("is_valid"), "Mermaid code should be valid"
...
B -> A: Result""",
    ...
    assert result.get("is_valid"), "D2 code should be valid"
    assert result.get("current_state") == "rendering", "Should move to rendering state"
...
@enduml""",
    ...
    assert result.get("is_valid"), "PlantUML code should be valid"
    assert result.get("current_state") == "rendering", "Should move to rendering state"
...
async def test_validate_invalid_code():
    ...
    mock_provider.validate_code.return_value = ValidationResult(
        is_valid=False, error="Invalid syntax", code_length=len(state["diagram_code"])
    )
...
async def test_render_mermaid_diagram():
    ...
    mock_provider.render_with_validation.return_value = RenderResult(
        success=True,
        content="<svg>...</svg>",
        output_format="svg",
