        ...
        if in_connection_label:
            ...
            if new_connection_match:
                ...
                if '"' in new_connection_match.group(3):
                    ...
                    fixed_lines.append(
                        f'{new_connection_match.group(1)} -> {new_connection_match.group(2)}: "{new_connection_match.group(3)}"'
                    )
...
def validate_d2_structure(code: str) -> List[str]:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            ":" in line
            and not object_def_pattern.match(line)
            and "->" not in line
            and not line.startswith("#")
            and not line.startswith("direction:")
            and "." not in line
        ):
