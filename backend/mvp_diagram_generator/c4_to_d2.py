...
def convert_c4_to_d2(c4_code: str) -> str:
    ...
    debug_msg = f"DEBUG: Found entities: {has_entities}, " f"relationships: {has_relationships}"
    ...
    for i, line in enumerate(lines):
        ...
        entity_pattern = r'^(\w+)\s*\(\s*(\w+)\s*,\s*"([^"]+)"' r'(?:\s*,\s*"([^"]*)")?(?:\s*,\s*"([^"]*)")?\s*\)'
        ...
        rel_pattern = r'^Rel(?:_[A-Z]+)?\s*\(\s*(\w+)\s*,\s*(\w+)\s*,\s*"([^"]+)"' r'(?:\s*,\s*"([^"]*)")?\s*\)'
    ...
    debug_msg = "DEBUG: Entities preserved in output (should be False): " f"{entities_preserved}"
    logger.debug(debug_msg)
...
def simple_c4_to_d2(c4_code: str) -> str:
    ...
    d2_code = re.sub(r'Person\s*\(\s*(\w+)\s*,\s*"([^"]+)"\s*\)', r'\1: {label: "\2"; shape: person}', d2_code)

    # Convert System() to D2 shape
    d2_code = re.sub(r'System\s*\(\s*(\w+)\s*,\s*"([^"]+)"\s*\)', r'\1: {label: "\2"; shape: rectangle}', d2_code)

    # Convert Rel() to D2 connection
    d2_code = re.sub(r'Rel\s*\(\s*(\w+)\s*,\s*(\w+)\s*,\s*"([^"]+)"\s*\)', r'\1 -> \2: "\3"', d2_code)
