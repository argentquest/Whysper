...
def fix_mermaid_syntax(code: str) -> MermaidFixResult:
    ...
    logger.debug(
        f"Mermaid syntax fixer: {len(corrections)} corrections, {len(warnings)} warnings, {len(errors)} errors"
    )
...
def fix_sequence_diagram_syntax(code: str) -> tuple[str, List[str]]:
    ...
    original = fixed
    fixed = re.sub(
        r'participant\s+(\w+)\s+as\s+(["\']?)([^"\'\n]+)\2', r"participant \1 as \3", fixed, flags=re.IGNORECASE
    )
    ...
    for line in lines:
        ...
        if stripped:
            ...
            if stripped.startswith("end"):
                ...
                stripped.startswith(kw) for kw in ["subgraph", "participant", "sequenceDiagram", "flowchart", "graph"]
            ):
                cleaned_lines.append(stripped)
            else:
                ...
                cleaned_lines.append(
                    "    " + stripped
                    if cleaned_lines and not cleaned_lines[-1].startswith(("flowchart", "graph", "sequenceDiagram"))
                    else stripped
                )
        else:
            cleaned_lines.append("")
