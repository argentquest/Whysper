...
const applyCssVariables = (token: Record<string, unknown>) => {
  ...
  const cssValues: Record<(typeof CSS_BRAND_VARS)[number], string> = {
    '--wf-header-bg': String(
      token.colorBrandHeaderBg ?? token.colorBrandHeader ?? token.colorPrimary ?? '#d71e28'
    ),
    ...
    '--wf-footer-card-bg': String(
      token.colorBrandFooterCardBg ?? token.colorBgContainer ?? '#ffffff'
    ),
    ...
    '--wf-link-color': String(
      token.colorBrandLink ?? token.colorLink ?? token.colorPrimary ?? '#004c97'
    ),
    '--wf-font-body': String(token.fontFamily ?? '"Wells Fargo Sans", Arial, sans-serif'),
    '--wf-font-heading': String(
      token.fontFamilyHeading ?? token.fontFamily ?? '"Wells Fargo Sans", Arial, sans-serif'
    ),
