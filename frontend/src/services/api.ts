...
export class ApiService {
  ...
  ): Promise<ApiResponse<{ message: string; conversationId: string }>> {
    try {
  ...
  static async getConversationFiles(
    conversationId: string
  ): Promise<ApiResponse<{ files: string[]; count: number }>> {
    try {
  ...
  static async updateEnvSettings(envUpdates: {
    [key: string]: string
  ...
  static async getAgents(): Promise<ApiResponse<unknown[]>> {
    try {
  ...
  static async getSubagents(): Promise<ApiResponse<unknown[]>> {
    try {
  ...
  static async post(url: string, data: unknown): Promise<AxiosResponse> {
  ...
  static async generateBulkDocumentation(request: {
    ...
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : 'Failed to generate bulk documentation'
      )
    }
  }

  static async getDocumentationTemplates(): Promise<
    ApiResponse<{
      templates: Array<{
        name: string
        title: string
   ...
   */
  static async checkAuthRequired(): Promise<
    ApiResponse<{ auth_required: boolean; auth_disabled: boolean }>
  > {
    try {
