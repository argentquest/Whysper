 ...
 */
const PLANTUML_C4_MARKERS = ['@startuml', '@enduml', '!include'] as const
...
export const isMermaidCode = (language: string, inline: boolean): boolean => {
  ...
  if (isMermaid) {
    console.log('🎨 [DIAGRAM DETECTION] Mermaid diagram detected (language marker)', {
      language,
      inline,
    })
    // Log to backend
...
export const isD2Code = (language: string, inline: boolean): boolean => {
  ...
  if (isD2) {
    console.log('🎯 [DIAGRAM DETECTION] D2 diagram detected (language marker)', {
      language,
      inline,
    })
    // Log to backend
...
export const isC4Code = (language: string, inline: boolean): boolean => {
  ...
  if (isDirectC4) {
    console.log('🏗️ [DIAGRAM DETECTION] C4 diagram detected (language marker)', {
      language,
      inline,
    })
    // Log to backend
  ...
  if (isPlantUML) {
    console.log(
      '🏗️ [DIAGRAM DETECTION] PlantUML detected (language marker), checking for C4 content',
      { language, inline }
    )
...
export const extractDiagramCandidates = (
  ...
  codeBlocks.forEach((codeEl) => {
    ...
    if (code.trim().length > 10) {
      // Only consider longer inline code blocks
      let type: 'mermaid' | 'd2' | 'unknown' = 'unknown'
 ...
 */
export const processMixedHtmlContent = (
  htmlContent: string
): {
  originalHtml: string
  diagrams: Array<{
