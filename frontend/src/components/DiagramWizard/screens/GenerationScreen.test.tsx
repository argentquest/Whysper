...
describe('GenerationScreen', () => {
  const mockProps = {
    ...
    clarifications: [{ question: 'What type of system?', answer: 'E-commerce' }],
    sseConnected: true,
  ...
  describe('Code Display and Editing', () => {
    ...
    it('should show copy button for code', () => {
      ...
      const copyButtons = screen
        .getAllByRole('button')
        .filter((btn) => btn.querySelector('svg') || btn.textContent?.includes('Copy'))
  ...
  describe('Export Functionality', () => {
    it('should call onExportClick when export button clicked', async () => {
      ...
      render(<GenerationScreen {...mockProps} status={{ status: 'completed' } as DiagramUpdate} />)

      // Find export button (might be in footer or with download icon)
      const buttons = screen.getAllByRole('button')
      const exportButton = buttons.find(
        (btn) => btn.innerHTML.includes('Download') || btn.textContent?.includes('Export')
      )
  ...
  describe('New Diagram Functionality', () => {
    it('should have new diagram button', async () => {
      ...
      const buttons = screen.getAllByRole('button')
      const newDiagramButton = buttons.find(
        (btn) => btn.textContent?.includes('New') || btn.textContent?.includes('Start')
      )
  ...
  describe('Model Change', () => {
    it('should call onChangeModel when model change triggered', async () => {
      ...
      const buttons = screen.getAllByRole('button')
      const changeModelButton = buttons.find(
        (btn) => btn.textContent?.includes('Change') && btn.textContent?.includes('Model')
      )
  ...
  describe('Status Display', () => {
    it('should show refinement alert during code refinement', () => {
      render(<GenerationScreen {...mockProps} status={{ status: 'refining' } as DiagramUpdate} />)
      expect(screen.getByText(/Code Refinement/)).toBeInTheDocument()
    })

    it('should show refinement alert during fallback fix', () => {
      render(
        <GenerationScreen {...mockProps} status={{ status: 'fallback_fix' } as DiagramUpdate} />
      )
      expect(screen.getByText(/Code Refinement/)).toBeInTheDocument()
    })
  ...
  describe('Model Display', () => {
    it('should display all model types', () => {
      ...
      models.forEach((model) => {
        const { unmount } = render(<GenerationScreen {...mockProps} selectedModel={model} />)
